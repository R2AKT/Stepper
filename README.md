Debug module. For connecting to the CPU_8080 processor board - https://github.com/R2AKT/CPU_8080.

Allows you to execute the program in step-by-step mode, displaying the states of the address and data buses.
Using the boot module, you can read and write data from/to memory - https://github.com/R2AKT/8080-5-CI.

Status: the step-by-step operation, the output of the status of the data buses and the address have been checked. Working with memory during testing.

Модуль отладки. Для подключения к процессорной плате CPU_8080 - https://github.com/R2AKT/CPU_8080.

Позволяет выполнять программу в пошаговом режиме, с отображением состояний шины адреса и данных.
С помощью модуля загрузки позволяет считывать и записывать данные из/в память - https://github.com/R2AKT/8080-5-CI.

Статус: проверена пошаговая работа, вывод статуса шин данных и адреса. Работа с памятью в процессе тестирования.
